    ...
    public class CompanyManagementService : AccountService, ICompanyManagementService
    {
        ...
        public async Task<bool> AddCompany(AddCompanyViewModel model)
        {
            ...
            try
            {
                ...
                var company = new CompanyModel()
                {
                    ...
                    PhotoUrl = UploadFile(model.UploadFile, Container.Company).Result,
                    Status = 1
                };
        ...
        public async Task<bool> UpdateCompany(Guid id, AddCompanyViewModel model)
        {
            try
            {
                ...
                company.PhotoUrl = await DeleteFile(model.DeleteFile, Container.Company, company.PhotoUrl);
                company.PhotoUrl += await UploadFile(model.UploadFile, Container.Company);
