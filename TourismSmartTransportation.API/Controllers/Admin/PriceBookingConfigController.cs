...
namespace TourismSmartTransportation.API.Controllers.Admin
{
    [ApiController]
    [Authorize]
    [Route(ApiVer1Url.Admin.PriceBookingConfig)]
    public class PriceBookingConfigController : BaseController
    {
        private readonly IPriceBookingServiceConfig _service;

        public PriceBookingConfigController(IPriceBookingServiceConfig service)
        {
            _service = service;
        }

        [HttpGet]
        public async Task<IActionResult> Search([FromQuery] PriceBookingServiceSearchModel model)
        {
        ...
        [HttpPost]
        public async Task<IActionResult> Create([FromBody] CreatePriceBookingServiceModel model)
        {
            return SendResponse(await _service.CreatePrice(model));
        }

        [HttpPut("{id}")]
        public async Task<IActionResult> Update(Guid id, [FromBody] UpdatePriceBookingServiceModel model)
        {
